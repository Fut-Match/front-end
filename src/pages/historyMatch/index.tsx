...
import { Calendar, Clock, Trophy, Eye, Search, Users, Target, Shield } from 'lucide-react';
...
const HistoryScreen: React.FC<HistoryScreenProps> = ({ onNavigate, onSelectMatch }) => {
  ...
  const filteredMatches = historicalMatches.filter(match => {
    const matchesFilter = activeFilter === 'todos' || match.category === activeFilter;
  ...
  const stats = {
    ...
    totalGoals: historicalMatches.reduce((acc, match) => acc + match.totalGoals, 0),
    avgDuration: Math.round(historicalMatches.reduce((acc, match) => acc + match.duration, 0) / historicalMatches.length),
    totalParticipants: historicalMatches.reduce((acc, match) => acc + match.participants, 0)
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 pb-20">
      {/* Discrete Page Title */}
      <div className="mb-6">
        <h1 className="text-lg font-medium text-gray-700 text-center">📊 Histórico</h1>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        <div className="bg-white rounded-lg p-4 shadow-sm">
          <div className="flex items-center justify-between mb-2">
            <div className="text-2xl font-bold text-[#F2442E]">{stats.totalMatches}</div>
      ...
      <div className="space-y-4">
        {filteredMatches.length === 0 ? (
          <div className="bg-white rounded-lg p-8 text-center shadow-sm">
            ...
            <h3 className="font-medium text-gray-900 mb-1">Nenhuma partida encontrada</h3>
            <p className="text-sm text-gray-600">Tente ajustar os filtros ou termo de busca</p>
          </div>
        ) : (
            ...
            <div key={match.id} className="bg-white rounded-lg shadow-sm overflow-hidden">
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center space-x-2">
                      <span className="font-medium text-gray-900 text-sm">{match.mvp.name}</span>
              ...
              <div className="p-4 bg-gray-50">
                <div className="grid grid-cols-3 gap-4 mb-3 text-center">
                  <div>
                    <div className="text-sm font-medium text-gray-900">{match.duration} min</div>
                    <div className="text-xs text-gray-600">Duração</div>
                  </div>
                  <div>
                    <div className="text-sm font-medium text-gray-900">{match.participants}</div>
                    <div className="text-xs text-gray-600">Jogadores</div>
                  </div>
                  <div>
                    <div className="text-sm font-medium text-gray-900">{match.totalGoals}</div>
